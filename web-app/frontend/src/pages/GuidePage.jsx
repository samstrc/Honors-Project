...
function Mark({ className = "h-7 w-7 text-[13px]" }) {
  return (
    <div className={`flex shrink-0 items-center justify-center rounded-full bg-accent font-bold text-[var(--card)] ${className}`}>
      •
    </div>
  );
}
        ...
        <div ref={scrollRef} onScroll={onScroll} className="min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-5 sm:px-5">
          {messages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center animate-fade-in">
              <Mark className="h-11 w-11 text-xl" />
